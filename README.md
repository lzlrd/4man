...
```sh
/plugin marketplace add lzlrd/4man
/plugin install 4man@4man
```
...
orchestrator fans out up to 5 Coders, then Testers, then the review — concurrently. Shared
files and migrations serialise. On a failure it loops Coder→Tester up to twice per unit,
then reports with the failures documented.
...
findings to the Reviewer. No `/security-review` available → the Reviewer does a focused
manual pass.
...
Review an arbitrary diff, branch, or PR through the same fan-out — read-only, no building:
...
Required Notice: Copyright 2026 Diab Neiroukh (<https://github.com/lzlrd>).
